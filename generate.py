...
from extra_recipes import recipes as new_recipes
from recipe_stories import stories
for recipe in recipes:
    recipe.update(stories.get(recipe['slug'], {}))
...
def shell(title,desc,body):
 return f'''<!doctype html><html lang="en"><head><meta charset="utf-8"><meta name="viewport" content="width=device-width,initial-scale=1"><title>{escape(title)} | The Everyday Table</title><meta name="description" content="{escape(desc)}"><link rel="icon" href="/favicon.svg" type="image/svg+xml"><link rel="stylesheet" href="/style.css"></head><body><header><div class="wrap"><a class="brand" href="/" aria-label="The Everyday Table home"><img src="/favicon.svg" alt="" width="42" height="42"><span>The Everyday Table</span></a><nav aria-label="Main navigation"><a href="/">Recipes</a><a href="/about/">About</a><a href="/contact/">Contact</a></nav></div></header><main>{body}</main><footer><div class="wrap"><div><div class="footer-identity"><img src="/favicon.svg" alt="" width="38" height="38"><strong>The Everyday Table</strong></div><small>Simple recipes for real life.</small></div><div><a href="/about/">About</a><a href="/contact/">Contact</a><a href="/privacy/">Privacy</a><a href="/terms/">Terms</a></div></div></footer></body></html>'''
...
for i,r in enumerate(recipes):
 ...
 presentation=('<section class="recipe-story"><h2>About this recipe</h2>'+''.join('<p>'+escape(text)+'</p>' for text in r['presentation'])+'</section>') if r.get('presentation') else ''
 body=f'''<article class="article"><a class="back" href="/">← All recipes</a><p class="meta">{r['category']}</p><h1>{r['title']}</h1><p class="lead">{r['intro']}</p><p class="author-line"><img src="/elba-adams.webp" alt="" width="38" height="38"> <span>Written by <a href="/about/">Elba Adams</a> · Mexico</span></p><div class="recipe-facts"><span><strong>Total time:</strong> {r['time']}</span><span><strong>Yield:</strong> {r['servings']}</span></div><div class="recipe-actions"><a href="#recipe" class="action">Jump to recipe</a><button type="button" class="action" onclick="window.print()">Print recipe</button></div><div class="recipe-banner" style="--card-bg:{bg};--card-color:{fg}">{r['title']}</div>{presentation}<section id="recipe"><h2>Ingredients</h2><ul>{ingredients}</ul><h2>Method</h2><ol>{steps}</ol></section><div class="note"><strong>Storage &amp; helpful tip</strong><br>{r['tips']}</div></article>'''
 write('recipes/'+r['slug'],shell(r['title'],r['intro'],body))
 import json
 data={'@context':'https://schema.org','@type':'Recipe','name':r['title'],'description':r['intro'],'author':{'@type':'Person','name':'Elba Adams','url':'https://www.everydaytablekitchen.com/about/'},'image':'https://www.everydaytablekitchen.com/'+r.get('image',({'lemon-chickpea-rice-bowls':'chickpea','tomato-white-bean-skillet':'beans'}.get(r['slug'],r['slug']))+'.webp'),'recipeCategory':r['category'],'recipeYield':r['servings'],'totalTime':'PT'+r['time'].split()[0]+'M','recipeIngredient':r['ingredients'],'recipeInstructions':[{'@type':'HowToStep','text':step} for step in r['steps']]}
...
write('about',shell('About Elba Adams','Meet the editorial voice of The Everyday Table and learn how the recipes are presented.','<div class="page"><h1>Meet Elba Adams</h1><div class="author-profile"><img src="/elba-adams.webp" width="360" height="360" alt="Illustrated portrait representing Elba Adams" loading="lazy"><div><p class="meta">The Everyday Table · Mexico</p><p class="lead">Elba Adams is the editorial name used for recipes on The Everyday Table.</p><p>The portrait is an illustration. The site focuses on familiar ingredients, clear steps and practical tips that help make everyday meals easier.</p></div></div><h2>About The Everyday Table</h2><p>Each recipe aims to answer the questions that matter at the stove: what to buy, how long it takes, what to do in each step and how to store leftovers. The focus is on accessible ingredients and practical meals you can return to again.</p><p>Cooking times are estimates; appliances and ingredient sizes vary. Check that food is properly cooked and follow safe storage practices in your own kitchen.</p><h2>How recipes are presented</h2><p>Each page lists ingredients, numbered steps and practical notes on substitutions or storage. Photos illustrate the finished dish; your result may vary with ingredients and equipment.</p><h2>How to use this site</h2><p>Browse the <a href="/">recipe collection</a>, choose a dish and follow the ingredients and numbered steps. Notes at the end of each recipe offer storage or preparation tips.</p></div>'))
...
for recipe in new_recipes:
    for route in ['', 'recipes/'+recipe['slug']]:
        target=p/route/'index.html'
        markup=target.read_text()
        markup=markup.replace(escape(recipe['title'])+'</div>', f'<img src="/{recipe["image"]}" alt="{escape(recipe["title"])}" loading="lazy"></div>', 1)
        target.write_text(markup)
...
for index, recipe in enumerate(recipes):
 ...
 points=''.join('<li><strong>'+escape(heading)+'.</strong> '+escape(detail)+'</li>' for heading,detail in recipe.get('guidance',recipe_help.get(slug,[])))
 ...
 if recipe.get('conclusion'):
  extra='<section class="recipe-conclusion"><h2>Final thoughts</h2><p>'+escape(recipe['conclusion'])+'</p></section>'+extra
...
.recipe-story,.recipe-conclusion{font-size:17px;line-height:1.8;color:#38463c}
.recipe-story h2,.recipe-conclusion h2{color:var(--ink)}
.recipe-story p,.recipe-conclusion p{margin:0 0 18px}
.recipe-conclusion{margin-top:42px;padding:2px 24px 7px;background:#f0f2e6;border-radius:12px}
@media(max-width:480px){.recipe-story,.recipe-conclusion{font-size:16px}.recipe-conclusion{padding:2px 18px 5px}}
.footer-identity{display:flex;align-items:center;gap:10px}
.footer-identity img{display:block;width:38px;height:38px;border:1px solid #bdcdb7;border-radius:10px}
.author-line{display:flex;align-items:center;gap:10px}
.author-line img{display:block;width:38px;height:38px;object-fit:cover;border-radius:50%}
''')

# Canonical URLs and share metadata for each public page.
site='https://www.everydaytablekitchen.com'
images={r['slug']:r.get('image',{'lemon-chickpea-rice-bowls':'chickpea.webp','tomato-white-bean-skillet':'beans.webp'}.get(r['slug'],r['slug']+'.webp')) for r in recipes}
for page in p.rglob('index.html'):
    relative=page.parent.relative_to(p).as_posix()
    ...
    picture=images.get(slug,'elba-adams.webp' if slug=='about' else 'chickpea.webp')
    image_url=site+'/'+picture
    tags=(f'<link rel="canonical" href="{url}">'
          f'<meta property="og:type" content="{"article" if relative.startswith("recipes/") else "website"}">'
          f'<meta property="og:site_name" content="The Everyday Table">'
          f'<meta property="og:title" content="{title}">'
          f'<meta property="og:description" content="{description}">'
          f'<meta property="og:url" content="{url}">'
          f'<meta property="og:image" content="{image_url}">'
          f'<meta name="twitter:card" content="summary_large_image">')
    page.write_text(markup.replace('</head>',tags+'</head>',1))

# An optional human-readable index for systems that choose to read llms.txt.
(p/'llms.txt').write_text('# The Everyday Table\n\n'
    'Approachable breakfast, dinner, side dish and dessert recipes with ingredients, steps and cooking notes.\n'
    'Writer profile: '+site+'/about/\n'
    'Full sitemap: '+site+'/sitemap.xml\n\n'
    '## Recipes\n'+''.join('- '+r['title']+': '+site+'/recipes/'+r['slug']+'/\n' for r in recipes))
