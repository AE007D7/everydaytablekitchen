...
(p/'favicon.svg').write_text('<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 64 64"><rect width="64" height="64" rx="13" fill="#274738"/><path d="M13 39c1 11 9 16 19 16s18-5 19-16H13Zm4-5h30" fill="#e2edb9"/><path d="M32 30c-9-11-5-19 3-22 0 8 8 13-3 22Z" fill="#a9d395"/></svg>')
def shell(title,desc,body):
 return f'''<!doctype html><html lang="en"><head><meta charset="utf-8"><meta name="viewport" content="width=device-width,initial-scale=1"><title>{escape(title)} | The Everyday Table</title><meta name="description" content="{escape(desc)}"><link rel="icon" href="/favicon.svg" type="image/svg+xml"><link rel="stylesheet" href="/style.css"></head><body><header><div class="wrap"><a class="brand" href="/">The Everyday Table</a><nav aria-label="Main navigation"><a href="/">Recipes</a><a href="/about/">About</a><a href="/contact/">Contact</a></nav></div></header><main>{body}</main><footer><div class="wrap"><div><strong>The Everyday Table</strong><small>Simple recipes for real life.</small></div><div><a href="/about/">About</a><a href="/contact/">Contact</a><a href="/privacy/">Privacy</a><a href="/terms/">Terms</a></div></div></footer></body></html>'''
...
for i,r in enumerate(recipes):
 ...
 body=f'''<article class="article"><a class="back" href="/">← All recipes</a><p class="meta">{r['category']}</p><h1>{r['title']}</h1><p class="lead">{r['intro']}</p><div class="recipe-facts"><span><strong>Total time:</strong> {r['time']}</span><span><strong>Yield:</strong> {r['servings']}</span></div><div class="recipe-actions"><a href="#recipe" class="action">Jump to recipe</a><button type="button" class="action" onclick="window.print()">Print recipe</button></div><div class="recipe-banner" style="--card-bg:{bg};--card-color:{fg}">{r['title']}</div><section id="recipe"><h2>Ingredients</h2><ul>{ingredients}</ul><h2>Method</h2><ol>{steps}</ol></section><div class="note"><strong>Storage &amp; helpful tip</strong><br>{r['tips']}</div></article>'''
 write('recipes/'+r['slug'],shell(r['title'],r['intro'],body))
 import json
 data={'@context':'https://schema.org','@type':'Recipe','name':r['title'],'description':r['intro'],'image':'https://www.everydaytablekitchen.com/'+({'lemon-chickpea-rice-bowls':'chickpea','tomato-white-bean-skillet':'beans'}.get(r['slug'],r['slug']))+'.webp','recipeCategory':r['category'],'recipeYield':r['servings'],'totalTime':'PT'+r['time'].split()[0]+'M','recipeIngredient':r['ingredients'],'recipeInstructions':[{'@type':'HowToStep','text':step} for step in r['steps']]}
